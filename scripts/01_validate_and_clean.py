...
class CADDataValidator:
    ...
    def reload_config(self, config_path: str | None = None) -> Dict[str, Any]:
        """Reload configuration from disk and rebuild dependent lookups."""
    ...
    def _build_esri_export(self, df: pd.DataFrame) -> pd.DataFrame:
        """Project a DataFrame into the ESRI-export schema."""
        esri_columns = [
    ...
    def _export_sample(self, sample_df: pd.DataFrame) -> None:
        """Persist the sampled records for downstream testing."""
    ...
    def _generate_validation_recommendations(self, results: dict) -> list:
        ...
        if unmapped:
            ...
            recommendations.append(
                f"INCIDENT MAPPING: {len(unmapped)} incident types not found in CallType_Categories.csv. "
                f"Examples: {sample_examples}"
            )
        ...
        if address_issues:
            ...
            recommendations.append(
                f"ADDRESS QUALITY: {count} address records flagged (top issue: {top_issue}). "
                f"Review CAD vs RMS/Geocode backfill."
            )
...
def validate_cad_dataset_with_sampling(
    ...
    Args:
        ...
        sampling_method (str): Sampling method ('stratified', 'systematic', 'random'). Defaults to 'stratified'.
        validator (CADDataValidator | None): Existing validator instance to reuse.
