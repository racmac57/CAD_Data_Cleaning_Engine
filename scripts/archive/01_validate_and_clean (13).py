...
class CADDataValidator:
    ...
    def _load_incident_mapping(self) -> pd.DataFrame:
        ...
        """
        mapping_path = r"C:\Users\carucci_r\OneDrive - City of Hackensack\09_Reference\Classifications\CallTypes\CallType_Categories.csv"
        try:
            df = pd.read_csv(mapping_path)
            return df[['Incident', 'Incident_Norm', 'Response_Type']].drop_duplicates()
        except FileNotFoundError:
            logger.error(f"Incident mapping file {mapping_path} not found. Using empty mapping.")
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def clean_partition(part):
            ...
            if 'Incident' in part.columns:
                part = part.merge(self.incident_mapping, on='Incident', how='left')
    ...
    def _validate_datetime_duration(self, df: pd.DataFrame, result: dict) -> dict:
        ...
        fields = ['Time of Call', 'Time Dispatched', 'Time Out', 'Time In']
        durations = {
            'response': pd.to_timedelta(df['Time Dispatched'] - df['Time of Call'], errors='coerce'),
            'out': pd.to_timedelta(df['Time Out'] - df['Time Dispatched'], errors='coerce'),
            'in': pd.to_timedelta(df['Time In'] - df['Time Out'], errors='coerce')
        }
        valid = pd.Series([True] * len(df), index=df.index)
        for duration in durations.values():
            valid &= duration >= pd.Timedelta(0)
        result.update({'passed': valid.sum(), 'failed': (~valid).sum()})
        if (~valid).any():
...
def sample_data():
    ...
    return pd.DataFrame({
        'ReportNumberNew': ['25-123456', '25-987654A', '25-123457', '25-123456B'],
        'Incident': ['Noise Complaint', 'ROBBERY', 'Task Assignment', 'ASSAULT'],
        'How Reported': ['PHONE', '9-1-1', 'INVALID', 'RADIO'],
        'FullAddress2': ['123 Main Street, Hackensack, NJ, 07601', 'INVALID', '456 Elm St, Hackensack, NJ, 07601', 'Main & Atlantic, Hackensack, NJ, 07601'],
        'PDZone': ['7', '5', '10', '6'],
        'Time of Call': ['2025-10-17 17:00:00', '2025-10-17 17:05:00', '2025-10-17 17:10:00', '2025-10-17 17:15:00'],
        'Time Dispatched': ['2025-10-17 17:02:00', '2025-10-17 17:06:00', '2025-10-17 17:12:00', '2025-10-17 17:14:00'],
        'Time Out': ['2025-10-17 17:03:00', '2025-10-17 17:07:00', '2025-10-17 17:13:00', '2025-10-17 17:16:00'],
        'Time In': ['2025-10-17 17:04:00', '2025-10-17 17:08:00', '2025-10-17 17:14:00', '2025-10-17 17:17:00'],
        'Time Spent': ['0 days 00:04:00', '0 days 00:03:00', '-0 days 00:01:00', '0 days 25:00:00'],
        'Officer': ['P.O. John Doe', None, 'P.O. Jane Doe', 'P.O. John Doe'],
        'Disposition': ['COMPLETE', 'ADVISED', 'INVALID', 'COMPLETE'],
        'Response Type': ['ROUTINE', 'EMERGENCY', 'ROUTINE', 'URGENT']
    })
